...
EOF
    ...
    if [ "$SSL_PROVIDER" != "none" ]; then
        log_success "Nginx configured for HTTPS on ${DOMAIN} using ${SSL_PROVIDER}"
    else
        log_success "Nginx configured for HTTP on ${DOMAIN}"
        log_warning "HTTPS is not active for ${DOMAIN}. Set CERTBOT_EMAIL or add a Cloudflare Origin certificate, then rerun the script."
    fi
}
